IF EXISTS (SELECT 1 FROM SYS.OBJECTS A WHERE A.[NAME] = 'PRC_PEDIDO_ATUALIZAR')
	DROP PROCEDURE PRC_PEDIDO_ATUALIZAR;
GO

CREATE PROCEDURE PRC_PEDIDO_ATUALIZAR
(
	@ID_PEDIDO INT,
	@STATUS VARCHAR(20)
)
...
BEGIN
	...
	UPDATE TB_PEDIDO SET [STATUS] = @STATUS WHERE ID_PEDIDO = @ID_PEDIDO

END

GO

IF EXISTS (SELECT 1 FROM SYS.OBJECTS A WHERE A.[NAME] = 'PRC_PEDIDO_CONSULTAR')
	DROP PROCEDURE PRC_PEDIDO_CONSULTAR;

GO

CREATE PROCEDURE PRC_PEDIDO_CONSULTAR
(
	@ID_USUARIO INT
)
AS
BEGIN
	...
	SELECT
		/* DADOS PEDIDO */ 
		A.ID_PEDIDO, A.ID_USUARIO, A.STATUS, A.VALOR AS VALOR_TRANSACAO, A.DATA_CADASTRO, 
		/* DETALHES TRANSACAO */
		B.ID_PRODUTO, B.QUANTIDADE, B.VALOR,
		/* DETALHES PRODUTO */
		C.NOME, C.DESCRICAO, C.URL
	FROM TB_PEDIDO A 
		INNER JOIN TB_PEDIDO_PRODUTO B ON A.ID_PEDIDO = B.ID_PEDIDO
